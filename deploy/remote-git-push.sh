...
echo "=== Git init ==="
git init
git config user.name "nathanxiangang-web"
git config user.email "nathanxiangang-web@users.noreply.github.com"
git add -A
git commit -m "v0.1: Python-native CodeartsBridge

- 15 modules, zero third-party dependencies (Python 3.10+ stdlib only)
- Four transports: local, ssh-shell, ssh, remote-worktree
- State machine: READY -> QUEUED -> STARTING -> RUNNING -> REVIEW_REQUIRED -> DONE
- Policy engine: JSON profiles, phase/check evaluation, approval gates
- Daemon: systemd service, poll loop, health monitoring
- CLI: 11 subcommands (bootstrap, doctor, status, create, dispatch, run, review, cancel, pause, resume)
- 67 tests, all passing
- Protocol-preserving: same task directory, state machine, and config format as PowerShell bridge"

echo "=== Add remote and push ==="
git remote add origin https://github.com/nathanxiangang-web/CodeartsBridge.git
git branch -M main
git push -u origin main 2>&1

echo "=== Tag v0.1 ==="
git tag -a v0.1 -m "v0.1: Initial Python release"
